...
#define MHT_ADC_ENABLE()						({ ADCSRA |= (1 << ADEN); })	// Enable ADC.
#define MHT_ADC_DISABLE()						({ ADCSRA &= ~(1 << ADEN); })	// Disable ADC.
	
#define MHT_ADC_START_CONVERSION()				({ ADCSRA |= (1 << ADSC); })	// Start of ADC Conversion.
#define MHT_ADC_STOP_CONVERSION()				({ ADCSRA &= ~(1 << ADSC); })	// Stop of ADC Conversion.

#define MHT_ADC_AUTO_TRIGGER_ENABLE()			({ ADCSRA |= (1  << ADATE); })	// Enable ADC Auto Trigger.
#define MHT_ADC_AUTO_TRIGGER_DISABLE()			({ ADCSRA &= ~(1 << ADATE); })	// Disable ADC Auto Trigger.

#define MHT_ADC_CHECK__CONVERSION_COMPLETED()	({ ADCSRA & (1 << ADIF); })		// It will be true if Conversion is finished, and will be false if not finished yet.
#define MHT_ADC_WAIT_CONVERSION_COMPLETED()		({ while( (ADCSRA & (1 << ADIF)) == 0 ); })	// Wait here until conversion is finished.

#define MHT_ADC_ENABLE_INTERRUPT()				({ ADCSRA |= (1 << ADIE); })	// Enable ADC Interrupt Conversion Complete.
#define MHT_ADC_DISABLE_INTERRUPT()				({ ADCSRA &= ~(1 << ADIE); })	// Disable ADC Interrupt Conversion Complete.

#define MHT_ADC_CLEAR_INTERRUPT_FLAG()			({ ADCSRA |= (1 << ADIF); })	// Clear ADC Complete Conversion Interrupt Flag.
	
#define MHT_ADC_READ_DATA_HIGH()				({ ADCH; })						// Read ADC_HIGH.
#define MHT_ADC_READ_DATA_LOW()					({ ADCL; })						// Read ADC_LOW.		
#define MHT_ADC_READ_DATA()						({ ADC;  })						// Read ADC.
